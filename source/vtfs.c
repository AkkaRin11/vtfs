...
static ssize_t ram_write(ino_t ino, const char __user *buf, size_t len, loff_t *ppos)
{
        ...
        if (*ppos + len > fi->content.allocated) {
                ...
                if (new_size > fi->content.allocated)
                        memset(
                            new_data + fi->content.allocated, 0, new_size - fi->content.allocated);
...
vtfs_lookup(struct inode *parent_inode, struct dentry *child_dentry, unsigned int flags);
static int vtfs_create(struct mnt_idmap *idmap,
                       ...
                       bool b);
static int
vtfs_link(struct dentry *old_dentry, struct inode *parent_dir, struct dentry *new_dentry);
static int vtfs_unlink(struct inode *parent_inode, struct dentry *child_dentry);
static struct dentry *vtfs_mkdir(struct mnt_idmap *idmap,
                                 struct inode *parent_inode,
                                 struct dentry *child_dentry,
                                 umode_t mode);
...
static struct inode *
vtfs_make_inode(struct super_block *sb, umode_t mode, ino_t ino, struct vtfs_file_info *fi)
{
...
vtfs_lookup(struct inode *parent_inode, struct dentry *child_dentry, unsigned int flags)
{
        ...
        inode = vtfs_make_inode(
            parent_inode->i_sb, fi->is_dir ? (S_IFDIR | 0777) : (S_IFREG | 0666), fi->ino, fi);
        if (!inode)
